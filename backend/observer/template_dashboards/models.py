...
class DashboardTemplate(BaseModel):
    ...
    def clean(self):
        ...
        if self.widget_configs:
            for i, widget in enumerate(self.widget_configs):
                ...
                required_widget_fields = ['type', 'config']
                for field in required_widget_fields:
                    if field not in widget:
                        raise ValidationError(f"Widget {i} missing required field: {field}")
                
                # Position can be either a dict with x,y or separate position_x, position_y fields
                has_position_dict = 'position' in widget and isinstance(widget['position'], dict)
                has_position_fields = 'position_x' in widget and 'position_y' in widget
                
                if not (has_position_dict or has_position_fields):
                    raise ValidationError(f"Widget {i} must have either 'position' dict or 'position_x'/'position_y' fields")
    ...
    def create_new_version(self, version, user, **kwargs):
        ...
        # Handle None values properly for fields that don't accept None
        layout_config = kwargs.get('layout_config')
        if layout_config is None:
            layout_config = self.layout_config.copy() if self.layout_config else {}
        
        widget_configs = kwargs.get('widget_configs')
        if widget_configs is None:
            widget_configs = self.widget_configs.copy() if self.widget_configs else []
...
class Dashboard(BaseModel):
    ...
    def create_from_template(self, template, user, name=None):
        ...
        configuration = {
            'layout': template.layout_config.copy() if template.layout_config else {},
            'widgets': template.widget_configs.copy() if template.widget_configs else [],
            'metadata': {
